...
@WebServlet("/ShowPreviousImages")
public class ShowPreviousImages extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// get album id and first showed image id
		...
		if(pageNumber <= 1 || pageNumber > numPages) {
			ErrorHandler.displayError(request, response, "Wrong current page number.");
			return;
		}
		...
		for(int i = 0; i < (pageNumber-2) * maxImages; i++) {
			images.remove(0);
		}
