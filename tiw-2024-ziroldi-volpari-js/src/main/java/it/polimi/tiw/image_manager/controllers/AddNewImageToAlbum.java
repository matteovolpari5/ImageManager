	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(user.getId()!= albumDAO.getUserIdByAlbumId(albumId)) {
				response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
				response.getWriter().println("You can't add images to other users' albums");
				return;
			}
		...
		try {
			...
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
			response.getWriter().println("Missing image");
			return;
		}
		...
		try { 
			...
			response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
			response.getWriter().println("Error while adding image.");
			return;
		}
